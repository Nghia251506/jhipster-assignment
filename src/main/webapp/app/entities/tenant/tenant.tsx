  ...
  return (
    <div>
      <h2 id="tenant-heading" data-cy="TenantHeading">
        <Translate contentKey="seocrawlerApp.tenant.home.title">Tenants</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="seocrawlerApp.tenant.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/tenant/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="seocrawlerApp.tenant.home.createLabel">Create new Tenant</Translate>
          </Link>
        </div>
      </h2>
        ...
        {tenantList && tenantList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="seocrawlerApp.tenant.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('code')}>
                  <Translate contentKey="seocrawlerApp.tenant.code">Code</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('code')} />
                </th>
                <th className="hand" onClick={sort('name')}>
                  <Translate contentKey="seocrawlerApp.tenant.name">Name</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('name')} />
                </th>
                <th className="hand" onClick={sort('contactEmail')}>
                  <Translate contentKey="seocrawlerApp.tenant.contactEmail">Contact Email</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('contactEmail')} />
                </th>
                <th className="hand" onClick={sort('contactPhone')}>
                  <Translate contentKey="seocrawlerApp.tenant.contactPhone">Contact Phone</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('contactPhone')} />
                </th>
                <th className="hand" onClick={sort('maxUsers')}>
                  <Translate contentKey="seocrawlerApp.tenant.maxUsers">Max Users</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('maxUsers')} />
                </th>
                <th className="hand" onClick={sort('status')}>
                  <Translate contentKey="seocrawlerApp.tenant.status">Status</Translate>{' '}
            ...
            <tbody>
              {tenantList.map((tenant, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`seocrawlerApp.TenantStatus.${tenant.status}`} />
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="seocrawlerApp.tenant.home.notFound">No Tenants found</Translate>
            </div>
          )
        )}
      </div>
