...
public interface CategoryRepository extends JpaRepository<Category, Long> {
    ...
    @Query(
        value = "select category from Category category left join fetch category.tenant",
        countQuery = "select count(category) from Category category"
    )
    Page<Category> findAllWithToOneRelationships(Pageable pageable);

    @Query("select category from Category category left join fetch category.tenant")
    List<Category> findAllWithToOneRelationships();

    @Query("select category from Category category left join fetch category.tenant where category.id =:id")
