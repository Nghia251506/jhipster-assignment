...
public interface TagRepository extends JpaRepository<Tag, Long> {
    ...
    @Query(value = "select tag from Tag tag left join fetch tag.tenant", countQuery = "select count(tag) from Tag tag")
    Page<Tag> findAllWithToOneRelationships(Pageable pageable);

    @Query("select tag from Tag tag left join fetch tag.tenant")
    List<Tag> findAllWithToOneRelationships();

    @Query("select tag from Tag tag left join fetch tag.tenant where tag.id =:id")
